...
import { normalizeGoogleDriveUrl, UPLOAD_DIR } from "../utils/helpers";

// __dirname setelah build berada di dist/
...
export const uploadLocalImage = async (req: Request, res: Response) => {
  ...
  // URL yang disimpan tetap /images/filename sesuai app.ts
...
export const updateImage = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // CASE C: old local → still local + upload new file
    ...
    await db.query(
      `UPDATE ps_images SET type = $1, url = $2, created_at = NOW() WHERE id = $3`,
      [type, finalUrl, id]
    );
  ...
  try {
    ...
    for (const item of order) {
      if (!item.id || item.position === undefined) {
        return res.status(400).json({ error: "Each item must contain { id, position }" });
      }
      ...
      await db.query("UPDATE ps_images SET position = $1 WHERE id = $2", [position, id]);
    }
